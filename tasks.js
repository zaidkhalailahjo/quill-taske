...
window.pendingTaskAttachment = null;
window.pendingTaskAttachmentType = null;
window.pendingTaskAttachmentName = null;
...
window.switchTaskTab = (tabName) => {
    ...
    if (tabName === 'active') {
        ...
        btnReports.className = 'font-bold text-sm whitespace-nowrap bg-white/20 px-3 py-1 rounded-full text-white hover:bg-white/30 transition';
    } else {
        btnActive.className = 'font-bold text-sm whitespace-nowrap bg-white/20 px-3 py-1 rounded-full text-white hover:bg-white/30 transition';
...
window.renderTasks = () => {
    ...
    if(!isCEO) tasksToRender = window.globalTasks.filter(t => t.assigneeId === window.currentUserData.uid || t.createdBy === window.currentUserData.uid);
    ...
    document.getElementById('totalTasksCountList').innerText = tasksToRender.length;
    ...
    tasksToRender.forEach(task => {
        ...
        if(task.checklists && task.checklists.length > 0) {
            let clItemsHtml = task.checklists.map((cl, idx) => `
                <div class="flex items-center gap-2 p-1 hover:bg-gray-100 dark:hover:bg-gray-600 rounded">
                    <input type="checkbox" class="custom-checkbox w-4 h-4 rounded-sm" ${cl.isCompleted ? 'checked' : ''} ${!canCheck || isPendingApproval || isCompleted ? 'disabled' : ''} onchange="window.toggleTaskChecklistItem('${task.id}', ${idx}, this.checked)">
        ...
        listBody.innerHTML += `
            <tr class="task-row-item hover:bg-gray-50 dark:bg-gray-700/50 transition-colors border-b border-gray-100 dark:border-gray-700 ${isCompleted ? 'opacity-60' : ''}" data-id="${task.id}" data-title="${window.escapeHTML(task.title)}" data-assignee="${window.escapeHTML(task.assigneeName)}">
                ...
                <td class="p-3 text-center">${isCEO || task.createdBy === window.currentUserData.uid ? `<button onclick="window.deleteTask('${task.id}')" class="text-red-400 hover:text-red-600"><i class="fa-solid fa-trash"></i></button>` : ''}</td>
            </tr>
            <tr id="task-details-tr-${task.id}" class="bg-gray-50/50 dark:bg-gray-800/50"><td colspan="7" class="p-0 border-0"><div id="task-details-${task.id}" class="slide-down"><div class="p-4 mr-10"><p class="text-sm text-gray-600 dark:text-gray-300 whitespace-pre-wrap">${window.escapeHTML(task.desc)}</p>${checklistHtml}</div></div></td></tr>
        `;
    });
};
...
window.deleteTask = async (id) => {
    if(window.currentUserData.role !== 'CEO' && !window.globalTasks.find(t => t.id === id && t.createdBy === window.currentUserData.uid)) return;
...
// جميع دوال إنشاء المهام وتقاريرها (مثل addTaskFormNew, updateTaskStatusFromCheckbox) موجودة بملف الـ HTML، فقط نقلنا الجزء الخاص بالرسم والتحكم
