...
window.showToast = (message, type = 'info') => {
    ...
    let colorClass = type === 'success' ? 'bg-green-500' : (type === 'warning' ? 'bg-yellow-500' : (type === 'error' ? 'bg-red-500' : 'bg-primary'));
...
window.addEventListener('hashchange', () => {
    ...
    const validSections = ['home-grid', 'attendance', 'leaves', 'drive', 'meetings', 'settings', 'expenses', 'trainees', 'office-inventory', 'warehouse-inventory', 'robots'];
    
    if (!window.hasPunchedInToday && window.currentUserData.role !== 'CEO' && hash !== 'attendance') {
        window.location.hash = 'attendance';
        window.showToast('عذراً، يجب تسجيل الحضور أولاً لفتح النظام.', 'warning');
        return;
    }
